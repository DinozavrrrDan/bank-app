...
/**
 * Тест проверяет сохранение категории расхода с определенным значением description
 */
@SpringBootTest
@AutoConfigureMockMvc
class ApiTestSaveCategory {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(ApiTestSaveCategory.class);
    ...
    @Test
    void apiTestSaveCategory() throws Exception {
        LOGGER.info("Тест проверяет сохранение категории расхода");
        ...
        String response = mockMvc.perform(post("/api/category")
                ...
                .getResponse()
                .getContentAsString();
        Long categoryId = new ObjectMapper().readTree(response).get("id").asLong();
        LOGGER.info("Успешно сохранили категорию с id {} и description {}", categoryId, DESCRIPTION);

        LOGGER.info("Успешно проверили сохранение категории расхода с определенным значением description");
    }
