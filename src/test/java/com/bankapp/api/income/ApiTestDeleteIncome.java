...
/**
 * Тест проверяет удаление дохода по id
 */
@SpringBootTest
@AutoConfigureMockMvc
class ApiTestDeleteIncome {
    private static final String LOGIN = RandomString.make(10);
    private static final String PASSWORD = RandomString.make(10);

    private static final Logger LOGGER = LoggerFactory.getLogger(ApiTestDeleteIncome.class);
    ...
    public void setUp() throws Exception {
        ...
        response = mockMvc.perform(post("/api/income")
                ...
                .getResponse()
                .getContentAsString();
        incomeId = objectMapper.readTree(response).get("id").asLong();
        LOGGER.info("Успешно сохранили доход с id {}", incomeId);
    }

    @Test
    void apiTestDeleteIncome() throws Exception {
        LOGGER.info("Тест проверяет удаление дохода по id");

        LOGGER.info("Удаляем доход с id {}", incomeId);
        mockMvc.perform(delete("/api/income/" + incomeId)
                        .header("Authorization", "Bearer " + accessToken))
                .andExpect(status().isOk());
        LOGGER.info("Успешно удалили доход");
